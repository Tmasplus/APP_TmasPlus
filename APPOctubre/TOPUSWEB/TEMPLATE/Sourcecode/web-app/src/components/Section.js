...
const Section = (props) => {
  ...
  const sectionItems = [
    {
      id: 1,
      icon:  <EmojiTransportationIcon style={{ color: 'red', fontSize: '30px', marginRight: 5 }} />,
      sentence1:t('pruduct_section_heading_1'),
      sentence: t('product_section_1'),
    },
    {
      id: 2,
      icon:  <AccessTimeIcon style={{ color: 'red', fontSize: '30px', marginRight: 8 }} />,
              ...
              return (
                <div style={{marginBottom: 55}} key={key}>
                  <Box
                    ...
                    boxShadow={3}
                    style={{ backgroundColor: color.White ===0 ? color.CAR_BOX_EVEN : color.CAR_BOX_ODD, textAlign:'center'}}
                  >
                    <img alt='Car' src={item.image} style={{ width: 90, height: 80 }} />
                    ...
                    <Box style={{ backgroundColor: 'white', height: 215, borderRadius: 25, padding: 10}} boxShadow={3}>
                      ...
                      <Typography style={{ marginTop: 8,fontWeight:'bold',fontSize:25 }}>{item.serviceType}</Typography>
                      :
                      <Typography style={{ marginTop: 8,fontWeight:'bold',fontSize:25 }}>{item.serviceType}</Typography>
                      }
                      <Typography style={{ marginTop: 5 }}>{'Tarifa Mínima'}:{item.min_fare}</Typography>
                      <Typography style={{ marginTop: 5, fontWeight: 700 }}>{item.typeService}</Typography>
                      <Button
                        variant="contained"
                        color="error"
                        ...
                        onClick={(e) => { e.preventDefault(); 
                          if(loggedIn){
                          navigate(((role && role === 'driver' )|| (calcEst && !showEst && !optionsRequired && (role === 'admin' || role === 'fleetadmin' || role === 'company' ))) ? '/bookings' : '/addbookings', 
                          { state: { carData: item }} )
                          }else{
                            navigate('/login')
                          }
                      ...
                      > 
                        { role === 'driver' || (calcEst && !showEst && !optionsRequired && (role === 'admin' || role === 'fleetadmin' || role === 'company'  )) ? t('info'): 'Solicita Ahora'}
                      </Button>
                    </Box>
                  </Box>
                </div>
  ...
  return (
    <div>
      ...
      <Box sx={{ flexGrow: 1, minHeight: '350px', backgroundImage: `url(${backImage})`, backgroundSize: 'cover'}}>
        <Typography variant="h3" fontWeight={400} style={{ color: 'white', textAlign: 'center', padding: 35}}>{t('Soluciones tecnológicas de movilidad')}</Typography>
        <Grid container className={classes.sectionGridContainer}>
            ...
            return (
              ...
              >
                <div style={{display:'flex'}}>
                  {item.icon}
                  <div>
                    <Typography variant="h5" style={{ color: 'white', fontWeight: 'bold'}}>{item.sentence1}</Typography>
