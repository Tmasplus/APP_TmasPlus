...
const addToWallet = async (uid, amount, description, transaction_id) => {
    ...
    if (snapshot.val()) {
        ...
        // Recuperar el walletHistory existente
        let walletHistorySnapshot = await admin.database().ref("walletHistory/" + uid).once("value");
        let walletHistory = walletHistorySnapshot.val() ? walletHistorySnapshot.val() : {};

        // Verificar si el transaction_id ya existe
        let transactionExists = false;
        for (let key in walletHistory) {
            if (walletHistory[key].transaction_id === transaction_id) {
        ...
        if (!transactionExists) {
            ...
            // Actualizar el saldo y añadir a walletHistory si el transaction_id no existe
            ...
            return false; // Opcionalmente, puedes retornar algo específico si la transacción ya existe
        }
    } else {
        return false;
    }
