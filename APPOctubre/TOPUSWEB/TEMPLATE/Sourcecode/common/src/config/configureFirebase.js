...
const createFullStructure = (app, db, auth, storage, config) => {
    return {
        ...
        reportIncidenceRef: ref(db, 'report_incidence'),
        ...
        driverDocsRef: (uid) => stRef(storage, `users/${uid}/licenseImage`),
        driverDocsRefBack: (uid) => stRef(storage, `users/${uid}/licenseImageBack`),
        cardIDEmpTransRLImageBKRef: (uid) => stRef(storage, `users/${uid}/cardIDEmpTransRLImageBK`),
        cardIDEmpTransRLImageRef: (uid) => stRef(storage, `users/${uid}/cardIDEmpTransRLImage`),
        SOATImageRef: (uid) => stRef(storage, `users/${uid}/SOATImage`),
        daviplataImageRef: (uid) => stRef(storage, `users/${uid}/daviplataImage`),
        cardPropImageRef: (uid) => stRef(storage, `users/${uid}/cardPropImage`),
        cardPropImageBKRef: (uid) => stRef(storage, `users/${uid}/cardPropImageBK`),
        verifyIdImageBkRef: (uid) => stRef(storage, `users/${uid}/verifyIdImageBk`),
        ...
        bookingListRef: (uid, role) => {
            if (role === 'customer') {
                return query(ref(db, 'bookings'), orderByChild('customer'), equalTo(uid));
            } else if (role === 'driver') {
                return query(ref(db, 'bookings'), orderByChild('driver'), equalTo(uid));
            } else if (role === 'fleetadmin') {
                return query(ref(db, 'bookings'), orderByChild('fleetadmin'), equalTo(uid));
            } else if (role === 'company') {
                return query(ref(db, 'bookings'), orderByChild('company'), equalTo(uid));
            } else {
                return ref(db, 'bookings');
            }
        },
