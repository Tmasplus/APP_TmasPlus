...
export const updateProfile = (updateData) => async (dispatch) => {
  ...
  if (updateData.verifyIdImageBk) {
    await uploadBytesResumable(verifyIdImageBkRef(uid), updateData.verifyIdImageBk);
    updateData.verifyIdImageBk = await getDownloadURL(verifyIdImageBkRef(uid));
  }
  if (updateData.SOATimage) {
    await uploadBytesResumable(SOATImageRef(uid), updateData.SOATimage);
    updateData.SOATimage = await getDownloadURL(SOATImageRef(uid));
  }

  if (updateData.cardPropImage) {
    await uploadBytesResumable(cardPropImageRef(uid), updateData.cardPropImage);
    updateData.cardPropImage = await getDownloadURL(cardPropImageRef(uid));
  }

  if (updateData.cardPropImageBK) {
    await uploadBytesResumable(cardPropImageBKRef(uid), updateData.cardPropImageBK);
    updateData.cardPropImageBK = await getDownloadURL(cardPropImageBKRef(uid));
  }

  if (updateData.ccEmpTransImage) {
    await uploadBytesResumable(ccEmpTransImageRef(uid), updateData.ccEmpTransImage);
    updateData.ccEmpTransImage = await getDownloadURL(ccEmpTransImageRef(uid));
  }

  if (updateData.cardIDEmpTransRLImage) {
    await uploadBytesResumable(cardIDEmpTransRLImageRef(uid), updateData.cardIDEmpTransRLImage);
    updateData.cardIDEmpTransRLImage = await getDownloadURL(cardIDEmpTransRLImageRef(uid));
  }
  if (updateData.cardIDEmpTransRLImageBK) {
    await uploadBytesResumable(driverDocsRefBack(uid), updateData.cardIDEmpTransRLImageBK);
    updateData.cardIDEmpTransRLImageBK = await getDownloadURL(driverDocsRefBack(uid));
  }
  if (updateData.daviplataImage) {
    await uploadBytesResumable(daviplataImageRef(uid), updateData.daviplataImage);
    updateData.daviplataImage = await getDownloadURL(daviplataImageRef(uid));
  }
