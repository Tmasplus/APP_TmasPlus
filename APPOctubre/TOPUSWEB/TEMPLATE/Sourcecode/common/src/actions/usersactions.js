...
export const updateLicenseImage = (uid, imageBlob, imageType) => async (dispatch) => {
  ...
  const {
    ...
    cardPropImageRef,
    cardPropImageBKRef,
    cardIDEmpTransRLImageRef,
    cardIDEmpTransRLImageBKRef,
    daviplataImageRef
  } = firebase;
  ...
  if (imageType === 'SOATimage') {
    await uploadBytesResumable(SOATImageRef(uid), imageBlob);
    let image1 = await getDownloadURL(SOATImageRef(uid));
    profile.SOATimage = image1;
  }
  if (imageType === 'cardPropImage') {
    await uploadBytesResumable(cardPropImageRef(uid), imageBlob);
    let image1 = await getDownloadURL(cardPropImageRef(uid));
    profile.cardPropImage = image1;
  }
  if (imageType === 'cardPropImageBK') {
    await uploadBytesResumable(cardPropImageBKRef(uid), imageBlob);
    let image1 = await getDownloadURL(cardPropImageBKRef(uid));
    profile.cardPropImageBK = image1;
  }

  if (imageType === 'cardOPmage') {
    await uploadBytesResumable(driverDocsRef(uid), imageBlob);
    let image1 = await getDownloadURL(driverDocsRef(uid));
    profile.cardOPmage = image1;
  }

  if (imageType === 'cardOPmageBK') {
    await uploadBytesResumable(driverDocsRefBack(uid), imageBlob);
    let image1 = await getDownloadURL(driverDocsRefBack(uid));
    profile.cardOPmageBK = image1;
  }


  if (imageType === 'ccEmpTransImage') {
    await uploadBytesResumable(ccEmpTransImageRef(uid), imageBlob);
    let image1 = await getDownloadURL(ccEmpTransImageRef(uid));
    profile.ccEmpTransImage = image1;
  }

  if (imageType === 'cardIDEmpTransRLImage') {
    await uploadBytesResumable(cardIDEmpTransRLImageRef(uid), imageBlob);
    let image1 = await getDownloadURL(cardIDEmpTransRLImageRef(uid));
    profile.cardIDEmpTransRLImage = image1;
  }
  if (imageType === 'cardIDEmpTransRLImageBK') {
    await uploadBytesResumable(cardIDEmpTransRLImageBKRef(uid), imageBlob);
    let image1 = await getDownloadURL(cardIDEmpTransRLImageBKRef(uid));
    profile.cardIDEmpTransRLImageBK = image1;
  }
  ...
  if (imageType === 'verifyIdImageBk') {
    await uploadBytesResumable(verifyIdImageBkRef(uid), imageBlob);
    let image1 = await getDownloadURL(verifyIdImageBkRef(uid));
    profile.verifyIdImageBk = image1;
  }

  if (imageType === 'daviplataImage') {
    await uploadBytesResumable(daviplataImageRef(uid), imageBlob);
    let image1 = await getDownloadURL(daviplataImageRef(uid));
    profile.daviplataImage = image1;
  }
