...
export default function AppCommon({ children }) {
    ...
    try {
      for (let i = 0; i < tasks.length; i++) {
        if (!playedSounds.includes(tasks[i].id)) {
          const notificationId = `booking_${i}`; // Generar un identificador único para cada notificación
          await Notifications.scheduleNotificationAsync({
            ...
            trigger: {
              seconds: 1, // Notificación mostrada 1 segundo después de ser programada
            },
          });
      ...
      console.error('Error en notifyBooking:', error);
    }
  };
  ...
  async function playSound() {
    const { sound } = await Audio.Sound.createAsync(require('./assets/sounds/car_horn_gap.wav'));
    setSound(sound);
    await sound.playAsync();
  }

  const notificationBooking = async () => {
    try {
      const notificaNewBooking = await Notifications.scheduleNotificationAsync({
        content: {
          title: t('notification_title'),
          body: t('new_booking_notification'),
          ios: {
        ...
        console.log('Error en notificationBooking:', error);
    }
  }
    ...
    if (auth.profile && languagedata && languagedata.langlist && settings && initialFunctionsNotCalled.current) {
      ...
      if (auth.profile.usertype) {
        ...
        if (role === 'customer') {
          ...
          dispatch(api.fetchCancelReasons());
          dispatch(api.fetchComplain());
          dispatch(api.fetchReportIncidents());
          initialFunctionsNotCalled.current = false;
