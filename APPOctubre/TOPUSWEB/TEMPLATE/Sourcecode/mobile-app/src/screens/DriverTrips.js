...
import { Text, View, StyleSheet, Dimensions, FlatList, Modal, TouchableHighlight, Switch, Image, Platform, Linking, TouchableOpacity, KeyboardAvoidingView, Pressable } from 'react-native';
...
import { ExtraInfo, RateView, appConsts, SECONDORY_COLOR } from '../common/sharedFunctions';
...
export default function DriverTrips(props) {
    ...
    useEffect(() => {
        if (auth.profile) {
            setChecks({
                ...
                profile_image: auth.profile.profile_image ? true : false,
                ...
                termsDaviplata: auth.profile.termsDaviplata ? true : false
            })
        }
    }, [auth.profile])
            ...
            if (!settings.negativeBalance && !settings.disable_cash && (wallet_balance <= 0 || (wallet_balance > 0 && wallet_balance < item.convenience_fees)) && item.payment_mode === 'cash') {
                ...
                if (parseFloat(price) > 0) {
                    ...
                    if (wallet_balance < convenience_fees && !settings.negativeBalance) {
                        ...
                        obj.vehicleColor = profile.vehicleColor ? profile.vehicleColor : "";
    ...
    const daviplataterms = () => {
        if (checks.termsDaviplata === false) {
            dispatch(updateProfile({ termsDaviplata: true }))
            dispatch(fetchTasks());
            setModalNotification(true)
        } else if (checks.termsDaviplata === true) {
        ...
        return (
            <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center' }}>
                ...
                <Switch
                    ...
                    trackColor={{ false: '#767577', true: '#F20505' }} // Cambia los colores del track
                    thumbColor={auth.profile.driverActiveStatus ? '#f4f3f4' : '#f4f3f4'} // Cambia el color del thumb
                    ios_backgroundColor="#3e3e3e" // Color de fondo en iOS cuando está desactivado
                />
            </View>
        );
    }
    ...
    const navchangueProfile = () => {
        props.navigation.dispatch(CommonActions.reset({ index: 0, routes: [{ name: 'Profile', params: { fromPage: 'DriverTrips' } }] }));
    }
                        ...
                        <View style={{ height: height, width: width }}>
                            ...
                            {gps.error || (!checks.carExists && settings.carType_required) || (!checks.carApproved && settings.carType_required) || (!checks.licenseAvailable && settings.license_image_required) || (!checks.profile_image) || (!checks.approved && settings.driver_approval) || !checks.driverActiveStatus || (settings && settings.imageIdApproval && !checks.imageIdApproval) || (!checks.term && settings.term_required) ?
                                <View style={{
                                    ...
                                    height: 10 + (gps.error ? 70 : 0) + (!checks.driverActiveStatus ? 70 : 0) + (!checks.carExists && settings.carType_required ? 70 : 0) + (!checks.carApproved && settings.carType_required ? 70 : 0) + (!checks.approved && settings.driver_approval ? 70 : 0) + (!checks.licenseAvailable && settings.license_image_required ? 70 : 0) + (!checks.profile_image ? 70 : 0) + (settings && settings.imageIdApproval && !checks.imageIdApproval ? 70 : 0) + (!checks.term && settings.term_required ? 70 : 0)
                                }}>
                                    {gps.error ?
                                    ...
                                    {!checks.licenseAvailable && settings.license_image_required ?
                                        ...
                                        : null}
                                    {!checks.profile_image && settings.license_image_required ?
                                        <View style={[styles.alrt, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                                            <View style={[styles.alrt1, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                                                <Icon name="alert-circle" type="ionicon" color={colors.RED} size={16} />
                                                <Text style={{ fontSize: 14, fontWeight: 'bold', color: colors.BLACK, marginLeft: 3 }}>{'terminos de daviplata'}</Text>
                                            </View>
                                            <Button onPress={navchangueProfile} title={t('profile')} titleStyle={styles.checkButtonTitle} buttonStyle={styles.checkButtonStyle} />
                                        </View>
                                        : null}
                    ...
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            <View style={{ margin: 10 }}>
                                <Image source={require("../../assets/payment-icons/daviplata-logo.png")} style={{ height: 135, width: 235 }} />
                            </View>
                            <Text style={styles.modalText}>Terminos de daviplata </Text>
                            <Pressable
                                style={[styles.button, styles.buttonClose]}
                                onPress={() => daviplataterms()}>
