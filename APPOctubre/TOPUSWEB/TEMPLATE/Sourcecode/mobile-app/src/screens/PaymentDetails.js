...
export default function PaymentDetails(props) {
  ...
  useEffect(() => {
    const fetchCommission = async () => {
      if (booking.company) {
        const commissionValue = await getCommission(booking.company);
  ...
  *///-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  const doPayment = (payment_mode) => {
    ...
    if (payment_mode == 'cash') {
      ...
      curBooking.trip_cost = curBooking.trip_cost || 0; // Establece 0 si trip_cost es undefined
      curBooking.customer_paid = curBooking.status == 'NEW' ? 0 : parseFloat(curBooking.trip_cost).toFixed(settings.decimal);
      ...
      if (curBooking.status === 'ACCEPTED') {
        ...
        curBooking.vehicleColor = curBooking.selectedBid.vehicleColor;
        ...
        curBooking.Iva = curBooking.selectedBid.Iva
        curBooking.driver_share = curBooking.trip_cost;
      ...
      curBooking.payment_mode = payment_mode;
      curBooking.customer_paid = curBooking.trip_cost.toFixed(settings.decimal)
      curBooking.discount = parseFloat(payDetails.discount).toFixed(settings.decimal);
      ...
      // Actualiza el estado de la reserva basado en su estado actual
      if (booking.status === "PAYMENT_PENDING") {
        curBooking.prepaid = true;
        //preguntarle a miguel porque se vuelve nuevo si se entra a este if 
      } else if (booking.status === "REACHED") {
        if (booking.prepaid || curBooking.booking_from_web) {
          curBooking.status = 'PAID';
        } else {
      ...
      // Calcula el incremento basado en el porcentaje de comisión
      const porcentajeIncrementoCorp = curBooking.Increase; // 30%
      const incrementoCorp = curBooking.trip_cost * porcentajeIncrementoCorp;

      // Calcular el incremento de ganacia de la empresa 
      const comisionEmpresarial = curBooking.CommisionHotel //10%
      const incrementoComision = curBooking.trip_cost * comisionEmpresarial

      curBooking.PolicyPackage = 800; // Valor fijo de paquete
      curBooking.cost_corp = curBooking.PolicyPackage + incrementoCorp.toFixed(2);
      curBooking.comisionCorp = incrementoComision.toFixed(2);
      console.log(curBooking.cost_corp);

      //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

      // Verifica si el valor de estimate es mayor que el de trip_cost
      if (parseFloat(curBooking.estimate) > parseFloat(curBooking.trip_cost)) {
        curBooking.trip_cost = curBooking.estimate;
      }

      curBooking.comisioCompany = (curBooking.trip_cost * 10 / 119).toFixed(2)
      curBooking.Iva = (curBooking.Iva * 0.19).toFixed(2)
      curBooking.Technological_Hosting = (curBooking.trip_cost * 0.3 / 1.19).toFixed(2)
      curBooking.Base_de_IVA = (curBooking.Technological_Hosting * 0.19).toFixed(2);

      //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      curBooking.status = curBooking.status === 'PAYMENT_PENDING' ? 'NEW' : curBooking.status;
      curBooking.status = curBooking.status === 'REACHED' ? (curBooking.prepaid || curBooking.booking_from_web || curBooking.payment_mode === 'corp' ? 'PAID' : 'PENDING') : curBooking.status;
      curBooking.status = curBooking.status === 'PENDING' ? 'PAID' : curBooking.status;
      curBooking.status = curBooking.status === 'NEW' ? 'ACCEPTED' : curBooking.status;
      // Realiza ajustes relacionados con el pago y otros campos
      ...
      curBooking.usedWalletMoney = 0;
      curBooking.cardPaymentAmount = 0;
      curBooking.corpPaymentAmount = curBooking.status === 'NEW' ? 0 : parseFloat(payDetails.amount - parseFloat(payDetails.discount)).toFixed(settings.decimal);
      curBooking.payableAmount = parseFloat(payDetails.payableAmount).toFixed(settings.decimal);
      curBooking.promo_applied = payDetails.promo_applied;
      curBooking.promo_details = payDetails.promo_details;
      // Asigna los detalles del conductor si el estado es 'ACCEPTED'
      if (curBooking.status === 'ACCEPTED') {
        curBooking.driver = curBooking.selectedBid.driver;
        curBooking.driver_image = curBooking.selectedBid.driver_image;
        curBooking.driver_name = curBooking.selectedBid.driver_name;
      ...
      // Realiza la actualización del booking
      setIsLoading(true);
      dispatch(updateBooking(curBooking));

      // Navega a la pantalla correspondiente después de la actualización
      setTimeout(() => {
        if (profile.usertype === 'customer') {
          if (curBooking.status === 'NEW' || curBooking.status === 'ACCEPTED') {
            props.navigation.navigate('BookedCab', { bookingId: booking.id });
    ...
    } if (payment_mode == 'Daviplata') {
      let curBooking = { ...booking };
      if (booking.status == "PAYMENT_PENDING") {
        curBooking.status = 'NEW';

      } else if (booking.status == "REACHED") {
        if (booking.prepaid || curBooking.booking_from_web || curBooking.payment_mode == 'Daviplata') {
          curBooking.status = 'PAID';
        } else {
          curBooking.status = 'PENDING';
        }
      ...
      curBooking.payment_mode = payment_mode;
      curBooking.customer_paid = curBooking.status == 'NEW' ? 0 : curBooking.trip_cost.toFixed(settings.decimal);
      curBooking.discount = parseFloat(payDetails.discount).toFixed(settings.decimal);
      curBooking.usedWalletMoney = 0;
      curBooking.cardPaymentAmount = 0;
      curBooking.DaviplataPaymentAmount = curBooking.status == 'NEW' ? 0 : parseFloat(payDetails.amount - parseFloat(payDetails.discount)).toFixed(settings.decimal);
      curBooking.payableAmount = parseFloat(payDetails.payableAmount).toFixed(settings.decimal);
      curBooking.promo_applied = payDetails.promo_applied;
      curBooking.promo_details = payDetails.promo_details;
    ...
    <View style={styles.mainView}>
      ...
      >
        ...
        >
              ...
              <View style={styles.cardPayBtnInnner}>
                {isLoading ? <ActivityIndicator size="small" color="white" /> :
                  <Text style={styles.buttonTitle}>
                    {booking.status == "PAYMENT_PENDING"
                      ? t("cash_on_delivery")
                      : booking.payment_mode == "Daviplata" ? "Daviplata" : t("complete_payment")}
                  </Text>
                }
              </View>
            </TouchableOpacity>
              ...
              <View style={styles.cardPayBtnInnner}>
                {isLoading ? <ActivityIndicator size="small" color="white" /> :
                  <Text style={styles.buttonTitle}>
                    {booking.status == "PAYMENT_PENDING"
                      ? t("cash_on_delivery")
                      : booking.payment_mode == "corp" ? "Pago Empresarial" : t("complete_payment")}
                  </Text>
                }
              </View>
            </TouchableOpacity>
          ...
          {providers && providers.length > 0 && booking.payment_mode == "corp" || booking.payment_mode == "card" || booking.payment_mode == "cash" || booking.payment_mode == "Daviplata" ?
            <TouchableOpacity
              style={styles.buttonWrapper3}
              onPress={() => {
                doPayment('card');
              }}>
              <Text style={styles.buttonTitle}>{profile && profile.usertype == 'customer' ? t('payWithCard') : t('request_payment')}</Text>
            </TouchableOpacity>
            : null}
