...
export default function DriverRating(props) {
    ...
    const skipRating = () => {
        ...
        const userId = curBooking.driver;  // Asegúrate de que 'booking' y 'driver' están definidos correctamente
        const walletHistoryRef = ref(database, `walletHistory/${userId}`);
        const walletBalanceRef = ref(database, `users/${userId}/walletBalance`);
        let transactionRemoved = false;
        get(walletHistoryRef).then(async (snapshot) => {
            if (snapshot.exists()) {
                
                const walletHistory = snapshot.val();
                
    
                for (const [key, transaction] of Object.entries(walletHistory)) {
                    if (transaction.txRef === curBooking.id && transaction.type === "Credit") {
                        try {
                            await remove(ref(database, `walletHistory/${userId}/${key}`));
                            console.log(`Transacción con txRef ${curBooking.id} eliminada con éxito.`);
                ...
                if(transactionRemoved){
                    ...
                    // Aquí podrías descomentar y usar estas líneas si necesitas actualizar la reserva y navegar
                ...
                console.log("No se encontraron datos de walletHistory.");
            }
        }).catch((error) => {
            console.error("Error al obtener datos: ", error);
        });
      
        get(walletBalanceRef).then(async (snapshot) => {
            if (snapshot.exists()) {
                let walletBalance = snapshot.val();
                console.log("Balance actual:", walletBalance);
        
                // Restar 'convenience_fees' del balance actual
                // Asegúrate de que 'convenience_fees' está definido y es un número
                walletBalance -= curBooking.convenience_fees;
        
                // Actualizar el nuevo balance en Firebase
                await set(walletBalanceRef, walletBalance)
                    .then(() => {
                        console.log('Wallet balance updated successfully with new value:', walletBalance);
                    })
                    .catch((error) => {
                        console.error("Error al actualizar el balance de la billetera:", error);
                    });
            } else {
                console.log("No se encontró el balance de la billetera para el usuario.");
            }
        }).catch((error) => {
            console.error("Error al obtener datos del balance de la billetera:", error);
        });
        // Actualiza la reserva actual
        props.navigation.navigate('TabRoot', { name: "RideList", params: { "fromBooking": true } });
    }
    ...
    const submitNow = () => {
        let curBooking = { ...booking };
                    ...
                    curBooking.rating = starCount;  // Asegúrate de que 'starCount' está definido correctamente
                    curBooking.feedback = state.feedback;  // Asegúrate de que 'state' está definido correctamente
                    curBooking.status = 'COMPLETE';
                    // Aquí podrías descomentar y usar estas líneas si necesitas actualizar la reserva y navegar
                    dispatch(updateBooking(curBooking));
                    // props.navigation.navigate('TabRoot', { name: "RideList", params: { "fromBooking": true } });
    ...
    return (
        <View style={[styles.mainViewStyle, { backgroundColor: colors.RED_TREAS }]}>
