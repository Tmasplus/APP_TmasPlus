...
import { MAIN_COLOR } from '../components/Button';
...
const DaviplataPayment = (props) => {
    ...
    const [payValues, setPayValues] = useState({
        documentType: auth.profile.docType || '',
        identificationNumber: auth.profile.verifyId || '',
        nameCustomer: `${auth.profile.firstName} ${auth.profile.lastName}` || '',
        buyValue: paymentDetail?.amount || 0
    })
...
//este es el que medio funciona 
    ...
    const buyTransactionDaviplata = async (accessToken) => {
        ...
        const documentType = payValues.documentType
        const buyValue = payValues.buyValue;
        const identificationNumber = __DEV__ ? identificationNumberMappingDev[documentType] : payValues.identificationNumber

        const daviplataDocumentType = typeDocumentMapping[documentType];
        console.log("identificacion", identificationNumber);
        const response = await axios.post(
            endpoint,
            {
                buyValue,
                identificationNumber,
                documentType: daviplataDocumentType,
                accessToken
            },
        );
        showModal()
    ...
    const readOtpInDevMode = async () => {
        ...
        const documentType = auth.profile.docType;
        const identificationNumber = __DEV__ ? identificationNumberMappingDev[documentType] : auth.profile.verifyId;
        const daviplataDocumentType = typeDocumentMapping[auth.profile.docType];
        const response = await axios.post(
            endpoint,
            {
                identificationNumber,
    ...
    const handleBuyButtonPress = async () => {
        ...
        try {
            ...
            // Mostrar el modal de OTP siempre
        ...
        <View style={styles.container}>
            ...
            {!confirmedPaymentResult ? <>
                ...
                <View style={styles.dataBuyContainer}>
                    ...
                    <TouchableOpacity style={styles.containerBotton} onPress={handleBuyButtonPress}>
                        <Text style={styles.textBuyButton}>Pagar</Text>
                    </TouchableOpacity>
                </View></> : <PaymentTransactionResult transactionResult={confirmedPaymentResult}
                    redirectSuccessTransaction={redirectSuccessTransactionRoute} />}
            ...
            <Modal isVisible={isModalPayDaviplataTransaction}>
                <View style={styles.modalView}>
                    ...
                    <View style={styles.buttonContainer}>
                        <TouchableOpacity style={styles.cancelButton}
                            onPress={() => {
                                setModalPayDaviplataTransaction(false); // Primero cerrar el modal
                                showMessageError({
                                    code: '401',
                        ...
                        <TouchableOpacity style={[
                            styles.sendButton,
                            valueCode.length < codeLength ? styles.disabledButton : null
                        ]} onPress={() => {
                            confirmBuyDaviplata({
                                otp: valueCode,
                                idSessionToken,
