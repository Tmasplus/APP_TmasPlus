...
import ChosenDriver from './ChosenDriver'
...
export default function TaxiModal(props) {
    ...
    const { settings, tripdata, estimate, bookingModalStatus, onPressCancel, bookNow, payment_mode, setPaymentMode, radioProps, profileData, setProfileData, auth, bookModelLoading, instructionData, setInstructionData, otherPerson, setOtherPerson, drivers } = props;
        ...
        <View>
            ...
            >
                <View style={styles.container}>
                    ...
                    <ScrollView style={{ height: (auth && auth.profile && auth.profile.firstName && auth.profile.lastName && auth.profile.email ? 250 : (auth.profile.firstName && auth.profile.lastName) ? 350 : auth.profile.email ? 350 : 400) + (otherPerson ? 75 : 10) + (ChosenDriver ? 75 : 10) }} showsVerticalScrollIndicator={false}>
                                ...
                                <View style={styles.priceDetailsLeft}>
                                    ...
                                    <View style={styles.iconContainer}>
                                        {settings.swipe_symbol === false ?
                                            <Text style={styles.priceText}> {settings ? settings.symbol : null} {estimate ? estimate.estimateFare : null} -  ${parseFloat(estimate ? estimate.estimateFare : 0) + 7000}  </Text>
                                            :
