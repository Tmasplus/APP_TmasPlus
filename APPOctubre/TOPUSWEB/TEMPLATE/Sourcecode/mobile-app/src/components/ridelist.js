...
export default function RideList(props) {
    ...
    const onAlert = (item) => {
        Alert.alert(t('alert'), t('booking_is') + ' ' + t(item.status) + "." + t('not_call'));
    }

    const onChatAlert = (item) => {
        Alert.alert(t('alert'), t('booking_is') + t(item.status) + "." + t('not_chat'));
    }
                ...
                <View style={[styles.box, { padding: 15 },]}>
                        ...
                        <View style={{ flexDirection: 'column', flex: 1 }}>
                            ...
                            <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                                <View style={{ width: 30, alignItems: 'center' }}>
                                    <Ionicons name="time" size={24} color={colors.BALANCE_GREEN} />

                                </View>
                                <View style={{ flex: 1, marginBottom: 10 }}>

                                    <View style={{ flexDirection: 'row' }}>
                                        <Text style={[styles.textStyle, isRTL ? { marginRight: 6, textAlign: 'right' } : { marginLeft: 6, textAlign: 'left' }]}>   {item.tripdate ? moment(item.tripdate).format('lll') : ''} </Text>
                                    </View>
                            ...
                            {item && ((item.driver_contact || item.customer_contact)) ?
                                <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center' }}>
                                    <TouchableOpacity onPress={() => (['ACCEPTED', 'ARRIVED', 'STARTED', 'REACHED', 'PENDING'].indexOf(item.status) != -1) ? role == 'customer' ?
                                        ...
                                        style={{ backgroundColor: '#FF9797', height: 40, width: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', margin: 3 }}>
                                        <Ionicons name="call" size={24} color={colors.WHITE} />
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => (['ACCEPTED', 'ARRIVED', 'STARTED', 'REACHED', 'PENDING'].indexOf(item.status) != -1) ? onChatAction(item, index) : onChatAlert(item)} style={{ backgroundColor: '#FF9797', height: 40, width: 40, borderRadius: 20, justifyContent: 'center', alignItems: 'center', margin: 3 }}>
                    ...
                    {(item && item.status && auth.profile && auth.profile.uid &&
                        ...
                        <Button
                            ...
                            style={{ backgroundColor: colors.RED_TREAS, marginVertical: 10 }}
...
const styles = StyleSheet.create({
    ...
    details: {
        ...
        alignItems: 'center',
        borderBottomWidth: .6,
        borderBottomColor: colors.RED_TREAS
    },
    clock: {
        flex: 1,
        justifyContent: 'space-around',
